...
/**
 *  Конструктор для базового использования
 *  column Номер столбца
 *  neighbor Указатель на соседа
 *  parent Родительский объект
 *
 * Инициализирует ферзя в первой строке (row = 1).
 */
QueenAgent::QueenAgent(int column, QueenAgent* neighbor, QObject* parent)
    ...
    , m_neighbor(neighbor)
    , m_maxRows(8)                  // Временное значение, будет установлено позже
    , m_terminal(false)
    , m_knowledgeBase(nullptr)
    , m_aggressiveness(0.5)         // Средняя агрессивность по умолчанию
...
bool QueenAgent::advanceWithKnowledge()
{
    ...
    // Самый левый ферзь достиг конца
    m_terminal = true;
    return false;
}
